...
export default class Login extends Component {
    ...
    handleSubmit = async (e) => {
        ...
        const res = query(collection(db, "it_service"), where("username", "==", username), where("password", "==", password));
        ...
        if (cek) {
            createUserWithEmailAndPassword(auth, 'xs@' + username + '.co', username).then(async (userCredential) => {
                // Signed in
                const uid = userCredential.user.uid;
                const update = doc(db, "it_service", id);
                ...
                if (error.code == 'auth/email-already-in-use') {
                    signInWithEmailAndPassword(auth, 'xs@' + username + '.co', username).then((user) => {
                        this.cekUser(user);
            ...
            signInWithEmailAndPassword(auth, 'xs@' + username + '.co', password).then((user) => {
                this.cekUser(user);
                if (!this.state.cekUser) {
                    $('.error-msg').removeAttr('hidden').text("Username atau password salah!");
                    $('.btn-submit').html('Log In').removeAttr('disabled');
                ...
                <div className="login-container" style={{ height: '100vh' }}>
                    <div className="login-box animated fadeInDown">
                        <div className="text-center">
                            <h2 style={{ color: '#fff', marginBottom: '20px' }}>Login Tim IT Service</h2>
                        </div>
